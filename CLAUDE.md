...
- **AI**: Google Gemini (`gemini-2.5-flash`), via the Vercel AI SDK (`ai` + `@ai-sdk/google`). Used for reply polishing (`server/src/lib/polish-reply.ts`), ticket summarizing (`server/src/lib/summarize-ticket.ts`), inbound-ticket classification (`server/src/lib/classify-ticket.ts`), and knowledge-base auto-resolution (`server/src/lib/auto-resolve-ticket.ts`); the API key and model come from `GEMINI_API_KEY` / `GEMINI_MODEL`.
- **Background jobs**: pg-boss (`server/src/queue/`), a PostgreSQL-backed durable job queue that reuses `DATABASE_URL` (it manages its own `pgboss` schema — no Prisma migration). The inbound-email webhook enqueues two independent jobs per new ticket — classification and auto-resolution — instead of running the models inline; workers started in `server/src/index.ts` consume the queues off the request path, so both are durable across restarts and retried (exponential backoff) on transient Gemini API failures. Enqueue on the request path, model calls in the workers.
...
## Outbound Email (SMTP / Mailpit)
- When an agent submits a reply (`POST /api/tickets/:id/replies`), the reply is persisted and then emailed to the ticket's `senderEmail` through the configured SMTP server, via `sendEmail()` in `server/src/lib/mailer.ts` (nodemailer). Subject is `Re: <ticket subject>`; the reply threads under the original message using the ticket's `sourceMessageId` (`inReplyTo`/`references`).
- Sending is **best-effort and off the response's critical path**: the reply is already saved, so a mail failure is logged but never fails the request (mirroring how the inbound webhook treats its enqueues). Config comes only from env vars — `SMTP_HOST`, `SMTP_PORT` (default 1025), `SMTP_USER`, `SMTP_PASS`, `SMTP_SECURE`, `MAIL_FROM` — never hardcoded.
- Email is **disabled when `SMTP_HOST` is unset**: `sendEmail()` no-ops without constructing a transport or touching the network, so the app (and tests, where no SMTP is configured) runs unchanged. This is the same graceful-degradation pattern as the AI features.
- **Local dev uses Mailpit** — a zero-config local SMTP server (SMTP on `1025`, web inbox on `http://localhost:8025`). No SendGrid, paid service, domain, or phone number needed. See `server/.env.example` for the exact vars.
- The **inbound webhook → pg-boss queue → worker → AI** flow is unchanged; the AI auto-resolution worker still posts its answer as an in-thread reply only (it does not email the customer).
