...
- When an agent submits a reply (`POST /api/tickets/:id/replies`), the reply is persisted and then emailed to the ticket's `senderEmail` through the Resend API, via `sendEmail()` in `server/src/lib/mailer.ts` (the `resend` SDK). Subject is `Re: <ticket subject>`; the reply threads under the original message using the ticket's `sourceMessageId`, sent as Resend custom `headers` (`In-Reply-To`/`References`).
- Sending is **best-effort and off the response's critical path**: the reply is already saved, so a mail failure is logged but never fails the request (mirroring how the inbound webhook treats its enqueues). Config comes only from env vars — `RESEND_API_KEY`, `MAIL_FROM` — never hardcoded.
- Email is **disabled when `RESEND_API_KEY` is unset**: `sendEmail()` no-ops without constructing a Resend client or touching the network, so the app (and tests, where no key is configured) runs unchanged. This is the same graceful-degradation pattern as the AI features.
- `MAIL_FROM` must be an address on a domain verified with Resend. See `server/.env.example` for the exact vars.
