...
## Testing
- Default to component tests. Only reach for E2E when the behavior genuinely can't be verified against mocked data — real auth/session flow, a route contract (headers, status codes, DB persistence) with no UI at all, or an interaction that spans multiple real routes/services in a way a single component can't fake. Pure UI rendering (list rows, badges/labels, detail fields, loading/error states, client-side navigation on click) belongs in a component test, even when the feature was originally built alongside an E2E flow — see `client/src/pages/TicketDetail.test.tsx` and the trimmed `e2e/tests/inbound-email-ticket.spec.ts` for the split.
