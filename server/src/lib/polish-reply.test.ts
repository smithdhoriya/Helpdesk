...
// `polishFailureReason` depends on. `@ai-sdk/google` is stubbed too, so module
// load doesn't try to construct a real provider — the returned "model" is
// opaque and only ever forwarded to the mocked `generateText`.
